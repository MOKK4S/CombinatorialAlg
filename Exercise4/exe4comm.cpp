#include <algorithm> // algorytmy standardowe (sort, max_element)
...
static bool solve(Counts& d, vector<int>& points, int L, Stats& stats) { // glowna funkcja rekurencyjna z przycinaniem
    stats.nodes++; // zlicz odwiedzony wezel
    if (d.empty()) return true; // jesli nie ma odleglosci, mamy rozwiazanie
    
    int y = d.rbegin()->first; // najwieksza pozostala odleglosc
    vector<int> removed; // lista usunietych odleglosci

    stats.candidates++; // pierwszy kandydat: punkt w pozycji y
    if (takeDistances(y, points, d, removed)) { // sprawdz zgodnosc odleglosci dla p = y
        points.push_back(y); // dodaj punkt do rozwiazania
    ...
    int p = L - y; // drugi kandydat (symetria wzgledem koncow)
    if (p != y) { // unikaj sprawdzania tego samego punktu
        stats.candidates++; // zlicz drugiego kandydata
        if (takeDistances(p, points, d, removed)) { // sprawdz zgodnosc odleglosci dla p = L - y
            points.push_back(p); // dodaj punkt do rozwiazania
            if (solve(d, points, L, stats)) return true; // rekurencyjnie szukaj dalej
            points.pop_back(); // cofniecie wyboru punktu
            restore(d, removed); // przywroc odleglosci po nieudanej probie
        } // koniec proby p = L - y
    } // koniec sprawdzania drugiego kandydata
